...
import pytest
from scripts.validate_external_blockers import BlockerValidationError, load_registry, validate_registry
...
def test_registry_is_valid_and_release_blocked():
    data=load_registry(REGISTRY); validate_registry(data,now=datetime(2026,9,16,tzinfo=timezone.utc))
...
def test_expired_evidence_is_rejected():
    ...
    with pytest.raises(BlockerValidationError,match="expired"): validate_registry(data,now=datetime(2026,9,16,tzinfo=timezone.utc))
