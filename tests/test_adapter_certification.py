...
def evidence(**changes):
    values = {
        "manifest": manifest(),
        "order_create_passed": True,
        "status_callback_passed": True,
        "duplicate_replay_passed": True,
        "conflicting_replay_blocked": True,
        "cancellation_passed": True,
        "fee_quote_passed": True,
        "pii_minimization_passed": True,
    }
    values.update(changes)
    return CertificationEvidence(**values)


def test_complete_adapter_is_certified():
    service = AdapterCertificationService()
    result = service.certify(evidence())
...
@pytest.mark.parametrize(
    "field", ["duplicate_replay_passed", "conflicting_replay_blocked", "pii_minimization_passed"]
)
