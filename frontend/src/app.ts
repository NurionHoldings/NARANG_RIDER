...
export async function mount(root: HTMLElement, session: SessionAdapter = new ServerSession()): Promise<void> {
  ...
  root.innerHTML = `<header><a class="brand" href="#main">나랑라이더 <small>NARANG RIDER</small></a><span role="status">합성 서버 연결</span></header>
  <main id="main"><h1>${titles[role]}</h1><p>서버 권한 검증이 최종 기준입니다.</p>
  <p id="form-help">각 작업은 한 번만 전송됩니다. 연결이 끊기면 내용을 확인한 뒤 직접 다시 시도하세요.</p>
  <div id="form-error" role="alert" tabindex="-1"></div><div id="form-status" aria-live="polite" tabindex="-1"></div>
  <section class="cards" aria-label="주요 업무">${journeys[role].map(action => `<form data-action="${action.id}" data-route="${action.route}" aria-describedby="form-help form-error"><h2>${action.label}</h2><label>처리 메모 <input name="note" autocomplete="off"></label><button type="submit">${action.label}</button></form>`).join("")}</section>
